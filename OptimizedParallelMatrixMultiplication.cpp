...
vector<vector<double>> OptimizedParallelMatrixMultiplication::transposeMatrix(vector<vector<double>> matrix, int dimensions) {
...
double OptimizedParallelMatrixMultiplication::computeDotProduct(vector<double> val1,vector<double> val2, int dimension) {
    double temp = 0.0;
    for(int i = 0; i < dimension; i++)
    {
        temp += val1[i] * val2[i];
